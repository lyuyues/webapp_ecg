...
@Component({
  selector: 'app-ecg-data-drag-zoom-2',
  templateUrl: './ecg-data-drag-zoom-2.component.html',
  styleUrls: ['../bootstrap-custom.css', '../common.css','./ecg-data-drag-zoom-2.component.css']
})
export class EcgDataDragZoom2Component implements OnInit {
  @Input() data2;
  ...
  this.maxY = this.d3.max(this.data2, function(d) { return d[1]; });
    ...
    this.g.append("text").attr("transform", "translate("+ (this.width-this.margin.left)/2 +"," + (this.height+this.margin.bottom) + ")")
          .text("Time (s)");
    var xExtent = d3.extent(this.data2, function(d) { return d[0]; });//[0, 299]
    this.zoom = d3.zoom()
        .scaleExtent([1 / 4, 8])
  ...
  this.areaPath.datum(this.data2)
               .attr("class", "area")
               .attr("d", this.area);
